 ...
 * 约定：
 *   - 每项配置为 { id, name, tag }
 *   - 路径自动拼为：src/cards/{id}/css/{id}-card.css、src/cards/{id}/js/{id}-component.js
...
(function () {
    ...
    config.forEach(function (card) {
        ...
        link.rel = 'stylesheet';
        link.href = 'src/cards/' + id + '/css/' + id + '-card.css';
    ...
    function loadScripts(idx) {
        ...
        script.src = 'src/cards/' + id + '/js/' + id + '-component.js';
